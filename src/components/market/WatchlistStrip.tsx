...
export function WatchlistStrip() {
  ...
  if (watchlist.length === 0) {
    return (
      <div className="flex items-center justify-between gap-3 rounded-card border border-dashed border-border-default bg-surface px-4 py-3 text-sm">
        <span className="text-text-secondary">
          Watch a player to track value from the day you add them.
        </span>
        <Link to="/board?sort=d1" className="shrink-0 text-brand-blue hover:underline">
  ...
  return (
    <div className="rounded-card border border-border-default bg-surface p-2">
      <div className="mb-1 flex items-center justify-between px-1">
        ...
        <Link to="/watchlist" className="text-[11px] text-brand-blue hover:underline">
          Open watchlist →
        </Link>
      </div>
        ...
        {entries.map((e) => {
          ...
          return (
            <Link
              ...
              className="flex shrink-0 items-center gap-2 rounded-control border border-border-default bg-canvas px-3 py-2 transition hover:border-brand-blue/40"
            >
              <span className="ticker text-xs font-semibold text-text-secondary">{e.row.player.ticker}</span>
              <span className="data text-sm tabnum text-text-primary">{e.row.snapshot.marketPrice.toFixed(1)}</span>
              <span className={cn('flex items-center gap-0.5 data text-[11px] tabnum', movementColor(e.delta))}>

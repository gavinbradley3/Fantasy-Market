...
export function PlayerMarketRow({ row }: { row: PlayerRow }) {
  ...
  return (
    <tr className="group border-b border-border-default transition hover:bg-elevated/50">
      <td className="py-2.5 pl-3">
        <Link to={`/player/${player.ticker}`} className="flex items-center gap-2.5">
          <span className="w-6 text-right data text-xs text-text-muted">{s.overallRank}</span>
      ...
      <td className="px-2 text-center data text-xs text-text-secondary">{player.age}</td>
      <td className="px-2 text-right data text-sm font-semibold text-text-primary">{fmtPrice(s.marketPrice)}</td>
...
export function PlayerMarketCard({ row }: { row: PlayerRow }) {
  ...
  return (
    <Link
      ...
      className="block rounded-card border border-border-default bg-surface p-3 transition-colors duration-standard hover:border-border-strong"
    >
      <div className="flex items-center gap-3">
        <span className="w-5 text-center data text-[11px] text-text-muted">{s.overallRank}</span>
        ...
        <div className="text-right">
          <div className="data text-data-lg font-semibold text-text-primary">{fmtPrice(s.marketPrice)}</div>
...
export function MoverRow({ row, metric = 'd1' }: { row: PlayerRow; metric?: 'd1' | 'd7' | 'd30' }) {
  ...
  return (
    <Link
      to={`/player/${player.ticker}`}
      className="flex items-center gap-3 rounded-control px-2 py-2.5 transition-colors duration-standard hover:bg-elevated"
    >
      <PlayerAvatar seed={player.avatarSeed} name={player.displayName} size={32} />
      <div className="min-w-0 flex-1">
        {/* min-w-0 has to repeat on this inner row: the name is `truncate`, which
            is white-space:nowrap, so without it this flex line reports the full
            untruncated name as its min-content and pushes the whole page wider
      ...
      <Sparkline data={row.spark} width={56} height={20} ariaLabel={`trend ${player.ticker}`} />
      <div className="flex w-[76px] shrink-0 flex-col items-end gap-0.5">
        <span className="data text-sm font-semibold text-text-primary">
          {fmtPrice(s.marketPrice)}
        </span>
        <MovementBadge value={s.movement[metric]} variant="plain" />
      </div>
    </Link>
  );
}
 ...
 * Titled mover panel.
 *
 * The panel itself stays quiet — a title, a line of context and rows. The old
 * coloured bullet beside each title is gone: nine panels each with a green or red
 * dot made the page read as a status board, and the direction is already stated in
 * the title and carried by every delta in the rows.
 */
export function MarketMoversPanel({
  ...
  return (
    // min-w-0 so the panel can shrink inside a grid track: grid items default to
    // min-width:auto, which would let a long player name widen the page.
    <section className="flex min-w-0 flex-col rounded-card border border-border-default bg-surface p-4">
      <div className="mb-2 flex items-baseline justify-between gap-2">
        ...
        {viewAllHref && (
          <Link
            to={viewAllHref}
            className="shrink-0 text-[11px] font-medium text-text-muted transition-colors duration-standard hover:text-brand-blue"
          >
            View all
          </Link>
        )}
      ...
      {rows.length > 0 ? (
        <div className="-mx-2 divide-y divide-border-default">
          {rows.map((r) => (
